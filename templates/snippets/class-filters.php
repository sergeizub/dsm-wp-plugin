...
<script>
	jQuery(function() {
		var hide_names = '<?php echo esc_js(DSM_OC_CLASS_FILTER_NAME_HIDDEN_IDS); ?>';
		if (hide_names != '') {		
		...
		var hide_programs = '<?php echo esc_js(DSM_OC_CLASS_FILTER_PROGRAM_HIDDEN_IDS); ?>';
		if (hide_programs != '') {
...
<?php if ($_SESSION['dsm_client_attrs'] && $_SESSION['dsm_client_attrs']['class_code']) : ?>
		<input type="hidden" id="filter_class_code" name="filter[class_code]" value="<?php echo esc_attr($_SESSION['dsm_client_attrs']['class_code']); ?>" />
<?php endif; ?>
<?php if ($_SESSION['dsm_client_attrs'] && $_SESSION['dsm_client_attrs']['class_name'] && $filters->name) : ?>
	<?php foreach ($filters->name as $name) : ?>
		 <?php echo (($name->label == $_SESSION['dsm_client_attrs']['class_name']) ? '<input type="hidden" id="filter_class_name" name="filter[class_name]" value="'.esc_attr($name->value).'"/>' : ''); ?>
	<?php endforeach; ?>
<?php elseif (DSM_OC_CLASS_FILTER_NAME == '1' &&  !$_SESSION['dsm_client_attrs']['class_code']) : ?>
	<div class="form-group">
		...
		<?php foreach ($filters->name as $name) : ?>
			<option value="<?php echo esc_attr($name->value); ?>" <?php echo (($name->value == $_POST['filter']['class_name']) ? 'selected="selected"' : ''); ?>><?php echo esc_attr($name->label); ?></option>
		<?php endforeach; ?>
		</select>
	</div>
<?php endif; ?>
<?php if ($_SESSION['dsm_client_attrs'] && $_SESSION['dsm_client_attrs']['class_level'] && $filters->level) : ?>
	<?php foreach ($filters->level as $level) : ?>
		 <?php echo (($level->label == $_SESSION['dsm_client_attrs']['class_level']) ? '<input type="hidden" id="filter_class_level" name="filter[class_level]" value="'.esc_attr($level->value).'"/>' : ''); ?>
	<?php endforeach; ?>
<?php elseif ((DSM_OC_CLASS_FILTER_LEVEL == '1') &&  !$_SESSION['dsm_client_attrs']['class_code']) : ?>
	<div class="form-group">
		...
		<?php foreach ($filters->level as $level) : ?>
			<option value="<?php echo esc_attr($level->value); ?>" <?php echo (($level->value == $_POST['filter']['class_level']) ? 'selected="selected"' : ''); ?>><?php echo esc_html($level->label); ?></option>
		<?php endforeach; ?>
		</select>
	</div>
<?php endif; ?>
<?php if ($_SESSION['dsm_client_attrs'] && $_SESSION['dsm_client_attrs']['class_location'] && $filters->location) : ?>
	<?php foreach ($filters->location as $location) : ?>
		 <?php echo (($location->label == $_SESSION['dsm_client_attrs']['class_location']) ? '<input type="hidden" id="filter_class_location" name="filter[class_location]" value="'.esc_attr($location->value).'"/>' : ''); ?>
	<?php endforeach; ?>
<?php elseif ((DSM_OC_CLASS_FILTER_LOCATION == '1' || $_SESSION['dsm_client_attrs']['show_location_filter']) &&  !$_SESSION['dsm_client_attrs']['class_code']) : ?>
	<div class="form-group">
		...
		<?php foreach ($filters->location as $location) : ?>
			<option value="<?php echo esc_attr($location->value); ?>" 
			<?php 
			...
			?>><?php echo esc_html($location->label); ?></option>
		<?php endforeach; ?>	
		</select>
	</div>
<?php endif; ?>
<?php if ($_SESSION['dsm_client_attrs'] && $_SESSION['dsm_client_attrs']['class_program'] && $filters->program) : ?>
	<?php foreach ($filters->program as $program) : ?>
		 <?php echo (($program->label == $_SESSION['dsm_client_attrs']['class_program']) ? '<input type="hidden" id="filter_class_program" name="filter[class_program]" value="'.esc_attr($program->label).'"/>' : ''); ?>
	<?php endforeach; ?>
<?php elseif (DSM_OC_CLASS_FILTER_PROGRAM == '1' && !$_SESSION['dsm_client_attrs']['class_code']) : ?>
	<div class="form-group">
		...
		<?php foreach ($filters->program as $program) : ?>
			<option value="<?php echo (($program->value) ? esc_attr($program->label) : ""); ?>" <?php echo (($program->label == $_POST['filter']['class_program']) ? 'selected="selected"' : ''); ?>><?php echo esc_html($program->label); ?></option>
		<?php endforeach; ?>
		</select>
	</div>
<?php endif; ?>
