...
?>
<script>
	window.account_credit =  '<?php echo esc_js($cart['balance'] * -1);?>';
	window.cart_total = '<?php echo esc_js($cart['total']);?>';
	jQuery(function() {	
...
<?php if (DSM_OC_SHOPPING_CART_PAYPAL == '1') : ?>
	...
	<div class="row">
		<div class="col-md-offset-6 col-sm-offset-5 col-md-5 col-sm-6">
		    ...
		    <form action="<?php echo esc_attr(DSM_PAYPAL_URL); ?>" method="post" id="gateway-form-checkout">
		        <input type="hidden" name="cmd" value="_xclick" />
		        <input type="hidden" name="business" value="<?php echo esc_attr(DSM_PAYPAL_EMAIL); ?>" />
		        <input type="hidden" name="locale" value="en_US" />
		        <input type="hidden" name="currency_code" value="<?php echo esc_attr(DSM_PAYPAL_PAYMENT_CURRENCY); ?>" />
		        <input type="hidden" name="item_name" value="Packages & Classes" />
		        <input type="hidden" name="item_number" value="" />
		        <input type="hidden" name="quantity" value="1" />
		        <input type="hidden" id="transaction_amount" name="amount" value="<?php echo esc_attr($cart['total']); ?>" />
				<input type="hidden" name="on1" value="Member ID">
		        <input type="hidden" name="os1" value="<?php echo esc_attr($cart['list']['0']['ID']); ?>">	
				<?php if ($cart['discount_coupon'] != "" ) : ?>
			    <input type="hidden" name="on2" value="Discount Coupon">
				<input type="hidden" name="os2" value="<?php echo esc_attr($cart['discount_coupon']); ?>">	
				<?php endif; ?>
		        <input type="hidden" name="return" value="<?php echo esc_url("//".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" />
		        <input type="hidden" name="cancel_return" value="<?php echo esc_url("//".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']); ?>" />
		        <input type="hidden" name="notify_url" value="<?php echo esc_url(get_option('dsm_api_url')); ?>ipn-paypal.php">
...
<?php else: ?>
	...
	<div class="row">
		<div class="col-md-10">
			...
			<input type="hidden" name="convenience_fee_percent" value="<?php echo esc_attr(DSM_CONVENIENCE_FEE_PERCENT); ?>">
		    <input type="hidden" name="convenience_fee_amount" value="<?php echo esc_attr(DSM_CONVENIENCE_FEE_AMOUNT); ?>">   
		    <input type="hidden" name="convenience_fee_for_check" value="<?php echo esc_attr(DSM_CONVENIENCE_FEE_FOR_CHECK); ?>">
			<input type="hidden" name="convenience_fee" value="<?php echo esc_attr($cart["convenience_fee"]);?>" id="convenience_fee_value">
			<input type="hidden" name="total_amount" value="<?php echo esc_attr($cart["total"]);?>" id="total_amount">   
			<?php if (!empty($selected_account)) :?>
			<input type="hidden" name="selected_account" value="<?php echo esc_attr($selected_account); ?>"/>
			<?php endif; ?>
			    <div class="form-group">
			        <label class="col-sm-5 control-label">Total Amount to Pay, <?php echo esc_attr(DSM_CURRENCY_SIGN); ?></label>
			        <div class="col-sm-7" id="transaction_amount_field">
			            <input class="form-control" type="text" readonly="readonly" id="transaction_amount" name="transaction_amount" style="width:100px;" value="<?php echo esc_attr($cart['total']); ?>">
			        </div>
			    </div>
			    ...
			    <div class="form-group row">
			        ...
			        <div class="col-sm-7">
			            <input type="checkbox" id="use_account_credit" name="use_account_credit"> Use Account Credit (<?php echo esc_html(DSM_CURRENCY_SIGN); echo esc_html($cart['balance'] * -1);?>)
			        </div>
			    </div>
			   <?php endif; ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						<select class="form-control" name="token_id" id="source_selector">
							...
							<option value="<?php echo esc_attr($token['id']); ?>" data-tender_type="<?php echo esc_attr($token['tender_type']);?>" <?php echo ((!empty($selected_account) && !empty($token['account_id']) && $selected_account != $token['account_id']) ? 'disabled="disabled"' : ''); ?>>
								<?php echo ((DSM_DSM_DATE_FORMAT == 'AU' && DSM_PAYMENT_SYSTEM == 'quickpay' && $token['tender_type'] == 'ACH') ? 'DD' : esc_attr($token['tender_type'])); ?> **** **** **** <?php echo esc_html($token['last4']); ?> <?php echo (($token['account']) ? '('.esc_html($token['account']).')': ''); ?>
		                    </option>
							<?php endforeach; ?>
						</select>
		        </div>
			...
			<div class="form-group">
			    ...
			    <div class="col-sm-7">
					<input class="form-control" type="text" name="card_number" value="<?php echo esc_attr(sanitize_text_field($_POST['card_number'])); ?>">
				</div>
			</div>
			<div class="form-group">
			    <label class="col-sm-5 control-label"><span class="text-warning">*</span> Card Expiration (MMYY)</label>
			    <div class="col-xs-3">
					<input class="form-control" type="text" name="card_expiration" maxlength="4" value="<?php echo esc_attr(sanitize_text_field($_POST['card_expiration'])); ?>">
				</div>
			</div>
			<?php endif; ?>
			</div>
		    <div id="card_cvv_info">
				...
				<div class="form-group">
			        ...
			        <div class="col-xs-3">
						 <input class="form-control" type="text" name="card_verification" maxlength="4" value="<?php echo esc_attr(sanitize_text_field($_POST['card_verification'])); ?>">
					</div>
			    </div>
				<?php endif; ?>
				...
				<div class="form-group">
					<label class="col-sm-5 control-label">&nbsp;</label>
					<div class="col-sm-7">
						<?php echo esc_attr(get_option('dsm_payment_notice')); ?>
					</div>      
				</div>
				<?php else: ?>
