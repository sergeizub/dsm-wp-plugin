		...
		<?php if (!empty($payment_sources)) : ?>
			...
			<tr>
				<td class="text-center">
						<input type="radio" name="default" value="<?php echo esc_attr($payment_source['id']); ?>" class="dsm_ajax_tab"
						dsm_obj="gateway" dsm_method="SubmitDefault" dsm_id="<?php echo esc_attr($payment_source['id']); ?>"
				...
				<td><?php echo esc_html($payment_source['tender_type']); ?> **** **** <?php echo esc_html($payment_source['last4']); ?></td>
				<td><?php echo esc_html($payment_source['description']); ?></td>
				<td>
				<select name="auto_payment"  class="form-control" name="auto_payment"
						dsm_obj="gateway" dsm_method="SubmitAutopay" dsm_id="<?php echo esc_attr($payment_source['id']); ?>" dsm_boot_tab="tab-gateway-finance" dsm_sub_tab="#tab-members-cards-accounts" dsm_auto_payment = "<?php echo esc_attr($payment_source['auto_payment']); ?>"
				...
				<?php if (DSM_OC_REMOVE_PAYMENT_SOURCE == '1') : ?>
				<td class="text-center">
					<a  href="#tab-gateway-finance" dsm_sub_tab="#tab-members-cards-accounts" dsm_obj="gateway" dsm_method="Delete"  dsm_id="<?php echo esc_attr($payment_source['id']); ?>" onclick="if (confirm('Are you sure you want to delete Card/ACH?')) { dsm_ajax_click(this) };return false;" title="Delete Card/ACH" class="btn btn-danger btn-sm"><i class="fa fa-minus-circle"></i> Remove</a>
				</td>
				<?php endif; ?>
			</tr>
			<?php endforeach; ?>
