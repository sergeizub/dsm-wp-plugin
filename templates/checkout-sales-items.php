...
<?php if (empty($sales_item_id)) : ?>
	<h2 class="page-header"><?php echo esc_html(DSM_OC_SALES_ITEMS_SECTION_TITLE); ?></h2>
<?php if (!empty($sales_products)) : ?>
	...
	<h3><?php echo esc_html($categories[$category_id]); ?></h3>
	<table class="table table-striped table-condensed table-hover">
		...
		<tr>
			...
			<td>
				...
				<?php echo ((DSM_IGNORE_ITEMS_AVAILABLE_QUANTITY === '0' && $product['AVAILABLE_QUANTITY'] && $product['AVAILABLE_QUANTITY'] > 0) ? '<div class="label label-warning">Only '.esc_html($product['AVAILABLE_QUANTITY']).' items available</div>' : ''); ?>
			</td>
			<td class="text-right"><?php echo esc_html(DSM_CURRENCY_SIGN);?><?php echo esc_html($product['PRICE']);?></td>
			<td class="text-right">
				...
				<?php if ($product['SALE_STARTED']) : ?>
					<?php if ($product['SALE_STARTED'] > 0 || DSM_IGNORE_ITEMS_AVAILABLE_QUANTITY == '1') : ?>	
						<?php if (App::GetClient()->GetController('auth')->isLogged()): ?>
							<a href="#tab-checkout-sales-items-<?php echo esc_html($product['ID']); ?>" title="Buy" class="btn btn-success dsm_ajax_tab"><i class="fa fa-shopping-cart"></i> Buy</a>
						<?php else: ?>
							<button dsm_sales-item_id="<?php echo esc_html($product['ID']); ?>" type="button" class="btn btn-success btn-login-alert"><i class="fa fa-shopping-cart"></i> Buy</button>
						<?php endif; ?>
					<?php else: ?>
						<div class="label label-default">Sold</div>
					<?php endif; ?>
				<?php else: ?>
					<div class="label label-warning">Sale starts <?php echo esc_html($product['SALE_START_DATE']); ?></div>
				<?php endif; ?>
			<?php endif; ?>
			</td>
		</tr>
